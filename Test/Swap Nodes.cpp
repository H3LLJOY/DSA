...
//    Node* left;
//    Node* right;
//    Node(int label , int depth ) : label(label),depth(depth),left(nullptr),right(nullptr){}
//};
//
//
//Node* buildTree(Node* root,vector<vector<int>> indexes){
//    Node* current=root;
//    queue <Node *> q;
//    q.push(current);
...
//        q.pop();
//        if(indexes[i][0]!=-1){
//            current->left= new Node(indexes[i][0],current->depth+1);
//            q.push(current->left);
//        }
//        if(indexes[i][1]!=-1){
//            current->right= new Node(indexes[i][1],current->depth+1);
...
//
//
//void storeInOrderTraversal(Node* root,vector<int>& result){
//    if(!root) return;
...
//        Node* root= buildTree(new  Node(1,1),indexes);
...
//            storeInOrderTraversal(root,tempResult);
